...
1. Add or extend a module core package for authoritative logic. `packages/game-core` stays generic; genre-specific logic belongs in a sibling package such as `packages/game-racer-core`.
2. Add a render/input package if the game needs a dedicated client runtime.
...
- If a game needs extra counters or content, add an extension table such as `racer_player_stats` instead of polluting `game_profiles`.
