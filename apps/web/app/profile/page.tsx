...
export default async function ProfilePage({
  ...
  return (
    <PageShell eyebrow="Profile" title="Shared Player Profile" description="Cross-game identity, progression, and wallet state stay outside the individual game package.">
      {!profile ? (
        <Card className="text-sm text-[var(--text-muted)]">Authenticate through Telegram to load your player profile.</Card>
      ) : (
        <>
          <section className="grid gap-4 md:grid-cols-3">
            <StatCard label="Level" value={String(profile.player.level)} hint={`${selectedProfile?.xp ?? 0} XP in selected game`} icon={<Gauge className="h-5 w-5" />} />
            <StatCard label="Coins" value={String(profile.wallet.coins)} hint="Wallet ledger is append-only" icon={<Coins className="h-5 w-5" />} />
            <StatCard label="Selected Game" value={selectedProfile?.gameName ?? "Unset"} hint={`Level ${selectedProfile?.level ?? 1} module progression`} icon={<Trophy className="h-5 w-5" />} />
          </section>

          <section className="grid gap-5 lg:grid-cols-[minmax(0,0.8fr)_minmax(0,1.2fr)]">
            <Card className="space-y-3">
              <p className="text-xs font-semibold uppercase tracking-[0.24em] text-[var(--accent-secondary)]">Identity</p>
              <div className="space-y-2 text-sm text-[var(--text-muted)]">
                <p>Name: {profile.player.displayNameSnapshot}</p>
                <p>Telegram ID: {profile.player.telegramUserId}</p>
                <p>Username: {profile.player.usernameSnapshot ?? "Not set"}</p>
                <p>Last seen: {new Date(profile.player.lastSeenAt).toLocaleString()}</p>
              </div>
            </Card>

            <Card className="space-y-3">
              <p className="text-xs font-semibold uppercase tracking-[0.24em] text-[var(--accent-secondary)]">Game progression</p>
              <div className="space-y-3">
                {profile.gameProfiles.map((entry) => (
                  <div key={entry.gameTitleId} className="rounded-[calc(var(--card-radius)-6px)] border border-[var(--border-subtle)] bg-[var(--surface-primary)] px-4 py-3 text-sm">
                    ...
                    <p className="text-[var(--text-muted)]">Level {entry.level} · {entry.xp} XP banked</p>
                  </div>
                ))}
              </div>
            </Card>
            ...
            <Card variant="glass" className="space-y-3">
              <p className="text-xs font-semibold uppercase tracking-[0.24em] text-[var(--accent-secondary)]">Recent wallet ledger</p>
              <div className="space-y-3">
                {profile.recentLedger.length === 0 ? (
                  <p className="text-sm text-[var(--text-muted)]">No official rewards have been granted yet.</p>
                ) : (
                  profile.recentLedger.map((entry) => (
                    <div key={entry.id} className="flex items-center justify-between rounded-[calc(var(--card-radius)-6px)] border border-[var(--border-subtle)] bg-[var(--surface-primary)] px-4 py-3 text-sm">
                      ...
                      <div className="text-right">
                        ...
                        <p className="text-[var(--text-muted)]">{new Date(entry.createdAt).toLocaleString()}</p>
                      </div>
                    </div>
                  ))
                )}
